...
ja_sentence_pattern = re.compile(r'(.+。+)')
tagger = MeCab.Tagger('--input-buffer-size {} --node-format %M[[[+++---]]] --unk-format %M[[[+++---]]]'.format(8192*640))
...
def convert(article_name,
    ...
    tokenized = re.split(ja_sentence_pattern, text_without_brackets)
    
    sentences = []
    for sent in tokenized:
        if sent != '':
            try:
                sentence = tagger.parse(sent).split('[[[+++---]]]')[:-1]
...
def main():
    ...
    with open(args.out, "wt") as fout:
        try:
            for i, (article_name, article) in tqdm(enumerate(iterate_articles(config.wiki))):
                if i == 5409:
                    continue
